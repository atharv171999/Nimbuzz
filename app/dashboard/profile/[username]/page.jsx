...
export default async function PublicProfilePage({ params }) {
    ...
    const [userPosts, followersCount, followingCount] = await Promise.all([
        getPostsByEmail(userProfile.email),
        getFollowerCount(userProfile.email),
        getFollowingCount(userProfile.email)
    ]);
    
    const isCurrentUser = session?.user?.email === userProfile.email;
    const isFollowing = session?.user?.email 
        ? await checkIsFollowing(session.user.email, userProfile.email) 
        : false;
