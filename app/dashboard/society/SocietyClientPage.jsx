...
export default function SocietyClientPage({ initialActivities, initialFilters, currentUserEmail }) {
    ...
    return (
        <div className="flex flex-col min-h-full w-full bg-zinc-50 relative p-6 md:p-12 transition-colors duration-500">
            
            {/* Header & Filter Bar */}
            <div className="max-w-6xl mx-auto w-full mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div className="flex flex-col gap-3">
                    <h1 className="text-5xl font-[family-name:var(--font-outfit)] font-black text-zinc-900 tracking-tighter">Society</h1>
                    <p className="text-zinc-400 font-bold uppercase tracking-[0.2em] text-[10px]">Discover local pings and join activities</p>
                </div>
            ...
            {/* Filter Section */}
            <div className="max-w-6xl mx-auto w-full mb-12 flex flex-col md:flex-row items-center gap-6 pb-12 border-b border-zinc-200/50">
                        ...
                        <button
                            ...
                            className={`px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all
                                ${initialFilters.category === cat 
                                    ...
                                    : 'bg-white text-zinc-500 border border-zinc-200/50 hover:border-primary/50'
                                }
                            `}
                        >
                            {cat}
                ...
                <div className="relative w-full md:w-80">
                    ...
                    <input 
                        ...
                        className="w-full bg-white border border-zinc-200/50 rounded-[1.5rem] py-4 pl-12 pr-6 text-sm font-black tracking-tight focus:ring-4 focus:ring-primary/10 focus:border-primary transition-all outline-none shadow-sm"
                    />
                </div>
            </div>
            ...
            <div className="max-w-6xl mx-auto w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {initialActivities.length === 0 ? (
                    <div className="col-span-full py-24 flex flex-col items-center text-center">
                        <div className="w-24 h-24 rounded-[2.5rem] bg-zinc-100 flex items-center justify-center mb-8 shadow-xl shadow-zinc-200/50 transition-transform hover:scale-110 duration-500">
                        ...
                        <h2 className="text-3xl font-[family-name:var(--font-outfit)] font-black text-zinc-900 mb-2 tracking-tighter">No Pings Yet</h2>
                        <p className="text-zinc-400 font-bold uppercase tracking-widest text-[10px]">Start an activity in your area</p>
                    </div>
                ) : (
